...
const modelosAutos = ["Gol", "Fiesta", "Suran", "208", "Polo", "Palio", "Fiesta", "Gol", "Captur", "Regatta", "Palio"];
function ultimaAparicionModeloAuto(modelo) {
    
    let posicionEncontrada = modelosAutos.lastIndexOf(modelo)
    
    if (posicionEncontrada !== -1) {

       console.log(`El modelo ${modelo} aparece por ulrima vez en la posicion ${posicionEncontrada + 1}`);
    
    } else { console.log(`El modelo ${modelo} no esta la lista`);
} 
    return posicionEncontrada
} 
console.log(ultimaAparicionModeloAuto("Ford"));