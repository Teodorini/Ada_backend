...
que agregue un nuevo libro al array libros.*/
 //Declaro un contadorId para que se incremente a medida que ingresamos nuevos datos
...
function agregarLibro (id, titulo, autor, anio, genero){
    ...
    libros.push(nuevoLibro);  // agrego el nuevo libro al final del array de libros con push
    console.log(libros);
    
};
// agregarLibro(11, "Los padecientes", "Rolon Gabriel", 2010, "Autoayuda");
...
function buscarLibro(criterio, valor) {
    ...
    for (let i = 0; i < libros.length; i++) {
        ...
        //verifico que el criterio sea igual al valor
        if (criterio === valor) {
...
function borrarLibro(id) {
    ...
    if (indice !== -1) {   //si el indice es diferente a -1, es true
// splice cambia el contenidoeliminando el indice encontrado
...
// console.log(buscarUsuario("melisa.espo@gmail.com"));
...
function devolverLibro(idLibro, idUsuario) {
    // con find busco el primer elemento que satisface la funcion. al tratarse de id unicos me parecio el mas conveniente de usar
    let libro = libros.find(libro => libro.id === idLibro);
    let usuario = usuarios.find(usuario => usuario.id === idUsuario);
    // condicion si se cumple se cambia a true
if (libro && !libro.disponible) {
